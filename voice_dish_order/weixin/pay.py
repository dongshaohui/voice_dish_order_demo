...
class Common_util_pub(object):
    ...
    def postXmlSSLCACurl(self, xml, url, second=30):
        """使用证书，以post方式提交xml到对应的接口url,并使用CA证书"""
        return HttpClient().postXmlCASSL(xml, url, second=second)
...
class Wxpay_client_pub(Common_util_pub):
    ...
    def postXmlSSLCA(self):
        """使用证书post请求xml,并包含CA证书"""
        xml = self.createXml()
        self.response = self.postXmlSSLCACurl(xml, self.url, self.curl_timeout)
        return self.response
...
class Redpack_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        # 检测必填参数
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
        self.parameters["mch_billno"] = WxPayConf_pub.MCHID + datetime.now().strftime('%Y%m%d') + (str)(time.time())[:10]
        self.parameters["mch_id"] = WxPayConf_pub.MCHID  #商户号
        self.parameters["wxappid"] = WxPayConf_pub.APPID  #公众账号ID     
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        print self.parameters
        xml_object = self.arrayToXml(self.parameters)
        print "Redpack_pub info ",self.parameters, xml_object
        return xml_object
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        self.parameters["sign"] = self.getSign(self.parameters)  #签名
        xml_object = self.arrayToXml(self.parameters)
        print "UnifiedOrder_pub info ",self.parameters, xml_object
        return  xml_object
    ...
    def getPrepayId(self):
        ...
        self.postXml()
        self.result = self.xmlToArray(self.response)
        print self.result," prepay result!!" , self.result['return_msg']
